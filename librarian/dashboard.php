    ...
    switch ($action) {
        case 'add_book':
            ...
            if (!empty($title) && !empty($author)) {
                try {
                    ...
                    $query = "INSERT INTO books (title, author, isbn, category, price) VALUES (?, ?, ?, ?, ?)";
                    $stmt = $db->prepare($query);
                    $stmt->execute([$title, $author, $isbn, $category, $price]);
        ...
        case 'update_book':
            ...
            if (!empty($book_id) && !empty($title) && !empty($author)) {
                try {
                    ...
                    $query = "UPDATE books SET title = ?, author = ?, isbn = ?, category = ?, price = ? WHERE id = ?";
                    $stmt = $db->prepare($query);
                    $stmt->execute([$title, $author, $isbn, $category, $price, $book_id]);
        ...
        case 'archive_book':
            ...
            if (!empty($book_id)) {
                ...
                try {
                    ...
                    $error_message = "Error archiving book: " . $e->getMessage();
                }
            }
            break;
        ...
        case 'unarchive_book':
            ...
            if (!empty($book_id)) {
                ...
                try {
                    $stmt->execute([$book_id]);
                    $success_message = "Book unarchived successfully!";
                } catch (PDOException $e) {
                    $error_message = "Error unarchiving book: " . $e->getMessage();
                }
            }
            break;
    }
